...
while True: #Ciclo infinito
    ...
    cuadro_ = cv2.resize(cuadro, (1920, 1080))

    detecciones = modelo_deteccion_placas.track(cuadro_, conf=0.61) #Detecta placas con un intervalo de confianza mayor a 0.61%
    ...
    for caja in cajas:
        ...
        if id != -1:   
            ...
            erosion_img = cv2.erode(threshold_img, (15,15), iterations=3)
            
            cv2.imshow('Placa', erosion_img) #Salida de prueba
            ...
            if numero_placa is not None:
                test = numero_placa
                #cuadro_numero_placa = cv2.putText(cuadro, test, (int(x1), int(y1)), cv2.FONT_HERSHEY_SIMPLEX, 6, (128, 17, 0), 7, cv2.LINE_AA)
                numero_placa_prev = test
                dict_placas[id] = test
    ...
    for key in dict_placas:
        cuadro_numero_placa = cv2.putText(cuadro, dict_placas[key], dict_coords[key], cv2.FONT_HERSHEY_SIMPLEX, 6, (128, 17, 0), 7, cv2.LINE_AA)
